...
public class LabeledItemSet extends ItemSet implements Serializable{
  ...
  public boolean similar(LabeledItemSet set){
	  if(set.m_classLabel!=m_classLabel)
		  return false;
	  if(!set.m_items.equals(m_items))
		  return false;
	  return true;
  }
   ...
   */  
    ...
    public static FastVector newsingletons(Instances instancesNoClass) throws Exception {
        ...
        for (int i = 0; i < instancesNoClass.numAttributes(); i++) {
    	  ...
    	  throw new Exception("Can't handle numeric attributes!");
          for (int j = 0; j < instancesNoClass.attribute(i).numValues(); j++){
    	      current = new LabeledItemSet(instancesNoClass.numInstances(),-1);
    	      current.m_items = new int[instancesNoClass.numAttributes()];
    	      for (int l = 0; l < instancesNoClass.numAttributes(); l++)
    		  current.m_items[l] = -1;
    	      current.m_items[i] = j;
...
//	  }
  ...
  public final void newupDateCounter(Instance instanceNoClass) {
      if (containedBy(instanceNoClass)){
	  m_counter++;
	  m_ruleSupCounter++;
      }
  ...
  public LabeledItemSet copy(){
	  
	  LabeledItemSet result=new LabeledItemSet(m_totalTransactions,m_classLabel);
...
//	  }
	  ...
	  result.m_ruleSupCounter=m_ruleSupCounter;
	  return result;
  }
  /**
   ...
   */  
   ...
   public static void newupDateCounters(FastVector itemSets, Instances instancesNoClass){

	    for (int i = 0; i < instancesNoClass.numInstances(); i++) {
	        Enumeration enu = itemSets.elements();
		while (enu.hasMoreElements())
	            ((LabeledItemSet)enu.nextElement()).newupDateCounter(instancesNoClass.instance(i));
	    }
    ...
    */   
    ...
    public final FastVector[] generateRulesII(double[] supB,double minConfidence, boolean noPrune) {
    	...
    	consequences.addElement(consequence);
    	conf.addElement(new Double((double)this.m_ruleSupCounter/(double)this.m_counter));
    	double Conf = (double)this.m_ruleSupCounter/(double)this.m_counter;
    	 if (Conf == 1) 
    		  Conf = 0.999;
    	double cov = (1 - supB[this.m_classLabel]) / (1 - Conf);
    	FastVector Conv = new FastVector();
    	Conv.addElement(new Double(cov));
    	rules[0] = premises;
    	rules[1] = consequences;
    	rules[2] = Conv;
    	if(!noPrune)
    ...
    public final FastVector[] newgenerateRules(double confidence,double conv) {

    	FastVector premises = new FastVector(),consequences = new FastVector(),
    	    conf = new FastVector();
    	FastVector Conv = new FastVector();
    	FastVector[] rules = new FastVector[4];
    	rules[0]= new FastVector();
    	rules[1]= new FastVector();
    	rules[2]= new FastVector();
    	rules[3]= new FastVector();
    	ItemSet premise, consequence;
    	...
    	Conv.addElement(new Double(conv));
    	rules[0] = premises;
    	rules[1] = consequences;
    	rules[2] = conf;
    	rules[3] = Conv; 
