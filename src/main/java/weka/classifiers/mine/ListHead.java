...
public class ListHead implements Serializable{
	static final long serialVersionUID = 7684467755712672058L;
	   ...
	   public void addNextII(LabelTnode t){
	   ...
	   public void addNextII(LabelTnode t,int[] rulesup){
		   for (int i = 0; i < sup.length; i++){
			   sup[i] += rulesup[i];
		   }
		   next.addElement(t);
	   ...
	   public boolean containedBy(LabelItemSet instance) {
		    for (int i = 0; i < instance.items().length; i++) 
		    {
		    	if (value != (int)instance.itemAt(attr))
			        return false;
		    }
		    return true;
		  }
		...
		public boolean equal(ListHead ch){
	    	if((value == ch.value) && (attr == ch.attr))
	    		return true;
	    	return false;
	    }
    ...
    public static void upDateCounters(FastVector itemSets, Instances instances) {
        ...
        for (int i = 0; i < instances.numInstances(); i++) {
        	Enumeration enu = itemSets.elements();
        	 while (enu.hasMoreElements()) {
		    	  ListHead node = (ListHead)enu.nextElement();
		    	 if(node != null){
    ...
    public static FastVector deleteItemSets(FastVector itemSets, 
    	...
    	Enumeration enu = itemSets.elements();
    	while (enu.hasMoreElements()) {
    		 ListHead current = (ListHead)enu.nextElement();
